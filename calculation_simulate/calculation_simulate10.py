...
from calculation_simulate.trick_module import *
from calculation_simulate.operate_input_string import *
...
class Calculator(object):
    def __init__(self, theme='classic', theme_list=theme_list):
        ...
        self.left_parentheses = 0
        self.right_parentheses = 0
    ...
    def press_comp_char(self, sign):  # 2
        ...
        if cond == 'n':
            ...
            self.record.set(' '.join(self.all_press_list))
            self.show_num.set('0')
            self.cur_cond = 'c'
    ...
    def press_equal_char(self, sign):  # 4
        if self.cur_cond == 'n':
            ...
            self.all_press_list.append(sign)
            self.all_press_list.append(str(eval(comp_string)))
            single_history_str = operate_history_single_str(self.all_press_list[:], self.input_number)
            self.history_record.set(self.history_record.get() + single_history_str)
    ...
    def press_coefficient(self, sign):  # 6
        # if self.is_computed:
        #     return
        if self.is_computed:
            self.press_clear_char('C')
        if sign == 'e':
            self.show_num.set(np.e)
        if sign == 'pi':
            self.show_num.set(np.pi)
        if sign == 'E_e':
            self.show_num.set(e)
            self.is_computed = True
        if sign == 'h':
    ...
    def operate(self, single_width=75, single_height=70, screen_height=200, c=7, r=6, text_list_2D=default_text_2D,
        ...
        if user_chose == [] or user_chose == ['0']:
            run_pictrue(self.root, user_chose)
            return
        ...
        self.input_number = int((total_width * 0.35) / (screen_height * 0.1))
        self.root.mainloop()
...
if __name__ == '__main__':
    ...
    my_calculator.operate(single_width=180, single_height=100, screen_height=250, user_chose=['0'])
